...
// import { Colours, setAlphaInColourString } from 'thaw-colour';
...
// 	private drawCandlestick(ohlcv: IExtendedOHLCVData, x: number): void {
// 		// Remember that coords (0, 0) is the *top* left corner of the canvas
// 		const yOpen = this.mapPriceToYCoord(ohlcv.open);
// 		const yHigh = this.mapPriceToYCoord(ohlcv.high);
// 		const yLow = this.mapPriceToYCoord(ohlcv.low);
// 		const yClose = this.mapPriceToYCoord(ohlcv.close);

// 		const candlestickColour =
// 			ohlcv.close >= ohlcv.open ? Colours.lime : Colours.red;

// 		this.canvasContext.fillStyle = candlestickColour;
// 		// rect(), fillRect(), strokeRect(), clearRect()
// 		this.canvasContext.fillRect(
// 			x + candlestickXMarginWidth,
// 			Math.min(yOpen, yClose),
// 			widthPerDataPoint - 2 * candlestickXMarginWidth,
// 			Math.max(Math.abs(yOpen - yClose), 1)
// 		);
// 		this.canvasContext.fillRect(
// 			x - 1 + widthPerDataPoint / 2,
// 			yHigh,
// 			2,
// 			yLow - yHigh
// 		);
...
// 	protected drawCandlesticks(ohlcvArray: IExtendedOHLCVData[]): void {
// 		ohlcvArray = ohlcvArray.slice(-this.canvasWidthInDataPoints);

// 		let x =
// 			(this.canvasWidthInDataPoints - ohlcvArray.length) *
// 			widthPerDataPoint;
...
// 	protected drawVolumeBars(ohlcvArrays: IExtendedOHLCVArrays): void {
// 		const canvasWidthInDataPoints = this.canvasWidthInDataPoints;
// 		const data = ohlcvArrays.volume.slice(-canvasWidthInDataPoints);
// 		const colours = pointwise(
// 			(o: number, c: number) => (c >= o ? Colours.lime : Colours.red),
// 			ohlcvArrays.open.slice(-canvasWidthInDataPoints),
// 			ohlcvArrays.close.slice(-canvasWidthInDataPoints)
// 		);
// 		const k = canvasWidthInDataPoints - data.length;

// 		this.beginDrawing(data);
...
// 			if (!Number.isNaN(datum)) {
// 				this.drawBar(
// 					(k + i) * widthPerDataPoint,
// 					datum,
// 					colours[i],
// 					colours[i] // setAlphaInColourString(colours[i], 0.5)
// 				);
// 			}
// 		}
// 	}
...
// 	protected drawHorizontalBarGraphForVolumeByPrice(
// 		vbpData: IVbpResult
// 	): void {
// 		// console.log(
// 		// 	'vbp 4: priceAtCanvasBottom is',
// 		// 	this.priceAtCanvasBottom
// 		// );
// 		// console.log('vbp 5: priceAtCanvasTop is', this.priceAtCanvasTop);

// 		const bottom = this.mapPriceToYCoord(vbpData.bottom);
// 		const top = this.mapPriceToYCoord(vbpData.top);

// 		// console.log('vbp 5a: bottom Y coord is', bottom);
// 		// console.log('vbp 5b: top Y coord is', top);
// 		// console.log('vbp 5c: canvasHeight is', this.canvasHeight);

// 		const zones = vbpData.volumes.length;
// 		const deltaYPerBar = Math.floor((bottom - top) / zones);

// 		if (!zones || bottom <= top || deltaYPerBar <= 0) {
// 			return;
// 		}

// 		const barOutlineColour = setAlphaInColourString(Colours.lime, 0.5);
// 		const barFillColour = barOutlineColour;

// 		// const deltaYPerBar = Math.floor(this.canvasHeight / zones);
// 		// let y = this.canvasHeight - deltaYPerBar;
// 		let y = bottom - deltaYPerBar;

// 		for (const volume of vbpData.volumes) {
// 			if (!Number.isNaN(volume)) {
// 				this.drawHorizontalBar(
// 					y,
// 					Math.round(volume * this.canvasWidth),
// 					deltaYPerBar,
// 					barOutlineColour,
// 					barFillColour
// 				);
